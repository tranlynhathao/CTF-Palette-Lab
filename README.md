...
CTF Palette Lab includes a dedicated **Adobe Illustrator Export** panel so designers can continue editing the wordmark in Illustrator without leaving fidelity on the table.

### Available exports

| Button                           | Output                                          | What it's for                                                                                                                                                                                                                                                                                                           |
| -------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Download SVG**                 | `hcmus-ctf-2026-logo-<slug>.svg`                | Editable vector SVG with the active palette baked in. Open directly in Illustrator (`File → Open`) or Figma. Brand pink stays exact.                                                                                                                                                                                    |
| **Download PDF**                 | `hcmus-ctf-2026-logo-<slug>.pdf`                | Vector PDF generated by `svg2pdf.js` + `jspdf` — every path is a real PDF vector operator, nothing is rasterised. Open in Illustrator and `Save As .ai`.                                                                                                                                                                |
| **Download Original AI**         | `logo-ctf26-original.ai`                        | Disabled by default in this public deployment — the master AI is held by the HCMUS CTF organisers and not committed to the public repo (see "Master AI policy" below). Re-enabled automatically if a private deployment serves the file at `assets/logo/logo-ctf26.ai`.                                                 |
| **Download Illustrator Package** | `hcmus-ctf-2026-illustrator-package-<slug>.zip` | Everything above bundled together with palette tokens (`palette.json`, `palette.css`, `tailwind-colors.js`, `figma-tokens.json`), an `illustrator-swatches.txt` ready to paste into the Illustrator Swatches panel, an optional `open-and-save-as-ai.jsx` automation script, and a `README.md` explaining the workflow. |

### Why no native `.ai` generation?

Adobe Illustrator's `.ai` format is proprietary and Adobe-controlled. A static, browser-only tool cannot honestly produce the same `.ai` file Illustrator itself produces, so CTF Palette Lab does **not** rename SVG/PDF to `.ai` and pretend. Instead it ships Illustrator-ready vector files (SVG, PDF) and lets a real `.ai` be one save away inside Illustrator.

### Master AI policy

**The master `logo-ctf26.ai` is intentionally not committed to this public repository.** It is held by the HCMUS CTF 2026 organising team and shared via internal channels (Drive / Slack), not via GitHub Pages.

Reasoning:

- The master file contains the editable layer / group structure of the wordmark. Publishing it to a public download URL would let anyone — including unaffiliated parties — remix or repurpose the brand asset.
- A public commit cannot be undone. Git history is permanent: deleting the file in a later commit still leaves it reachable via `git log` / `git checkout <old-sha>`.
- The on-screen logo preview, SVG export, and PDF export do **not** depend on the `.ai`. They are driven by `src/assets/logo/logo-ctf26.svg` (a flat, palette-aware vector extract) and remain fully functional whether or not the master is present.

In the public deployment:

- **Download Original AI** is disabled with the copy _"Master AI template is held by HCMUS CTF organizers. Contact the team for access."_
- The ZIP package substitutes `logo-ctf26-original.ai.NOTICE.txt` (a short policy note) for the missing `.ai`.
- `*.ai` is in `.gitignore` so a stray copy at any path can never accidentally land in a commit.

If you run a **private / internal** deployment of CTF Palette Lab and want the AI download enabled, drop the master at `public/assets/logo/logo-ctf26.ai` before `npm run build`. The button will light up automatically and the ZIP will bundle the AI without any code changes.

### Recommended workflow

1. Pick a palette in CTF Palette Lab.
2. Click **Download Illustrator Package**.
3. Open `logo-current-palette.svg` (or `.pdf`) in Adobe Illustrator.
4. Paste hex values from `illustrator-swatches.txt` into Illustrator's Swatches panel.
5. Edit, then `File → Save As → Adobe Illustrator (.ai)`.

### Brand color rule

`#E42175` is the fixed HCMUS CTF 2026 brand pink. CTF Palette Lab never recolours it in any export — only the dark "ink" outline and the back face of the extruded letterforms reflect the active palette. Text is exported as outlined paths (extracted once from the original `.ai`), which preserves exact visual fidelity even when Illustrator does not have the original font installed.

### Honest limitations

- The PDF export uses `svg2pdf.js`, which supports our SVG features (paths, fills, strokes, transforms, `<use>` references). Filters / masks / CSS variables are not supported there — that is why the export pipeline first flattens the recolorable SVG to literal hex values.
- The master `.ai` is deliberately unavailable in the public deployment (see "Master AI policy" above). SVG and PDF exports are unaffected.
